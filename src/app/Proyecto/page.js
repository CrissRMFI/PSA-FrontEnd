...
export default function ProyectosPage() {
  ...
  const handleUpdateProyecto = async (proyectoData) => {
    try {
      const updatedProyecto = await proyectosService.updateProyecto(editingProyecto.idProyecto, proyectoData);

      if (proyectoData.liderRecursoId && proyectoData.liderRecursoId !== editingProyecto.liderRecursoId) {
        await proyectosService.asignarLiderRecurso(editingProyecto.idProyecto, proyectoData.liderRecursoId);
      } else if (!proyectoData.liderRecursoId && editingProyecto.liderRecursoId) {
        await proyectosService.removerLiderRecurso(editingProyecto.idProyecto);
      }

      const proyectoFinal = await proyectosService.getProyectoById(editingProyecto.idProyecto);
      setProyectos((prev) =>
        prev.map((p) => (p.idProyecto === editingProyecto.idProyecto ? proyectoFinal : p))
      );
      setEditingProyecto(null);
      setShowForm(false);
      console.log('Proyecto actualizado exitosamente:', proyectoFinal);
    } catch (err) {
  ...
  const handleDeleteProyecto = async () => {
    try {
      ...
      setProyectos((prev) => prev.filter((p) => p.idProyecto !== deletingProyecto.idProyecto));
      ...
      <div className="mb-8">
        <div className="relative max-w-md">
          ...
          {searchTerm && (
            ...
            >
              <svg className="h-4 w-4 text-gray-400 hover:text-gray-600" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
        ...
        <div className="text-center py-12">
          {searchTerm ? (
            ...
            <>
              ...
              <h3 className="text-lg font-medium text-gray-800">No hay proyectos</h3>
